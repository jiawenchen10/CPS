...
class InnerProductDecoder(Layer):
    ...
    def __init__(self, cevae, sampled_nodes, dropout = 0., act = tf.nn.sigmoid, **kwargs):
        super(InnerProductDecoder, self).__init__(**kwargs)
        self.dropout = dropout
        self.act = act
        self.sampled_nodes = sampled_nodes # Nodes from sampled subgraph to decode
        self.cevae = cevae # Whether to use the cevae framework

    def _call(self, inputs):
        inputs = tf.nn.dropout(inputs, 1 - self.dropout)
        # If cevae is used, we only reconstruct the sampled subgraph
        if self.cevae:
